...
globals[time-recreate-requests]
breed[cars car]
...
cars-own[isavailable time-finish-request]                                          ;;if the car is available

turtles-own[myId incoming-queue]                                              ;;ID of requests
...
to setup
  ...
  set time-recreate-requests 5      ;;need set other variables maybe need to set input nb

  set-default-shape cars "car"
  create-cars initial_Nb_car  ;; create the cars, then initialize their variables
  [
    set color white
  ...
  create-requests initial_Nb_requests  ;; create the requists, then initialize their variables
  [
    set color green
    set size 1.5  ;; easier to see
    set label-color blue - 2
    ...
    set destination-x random-float 10                  ;;may be negative should change
    set destination-y random-float 10

    if ((random-float 1) > 0.5) [set destination-x (0 - destination-x)]
    if ((random-float 1) > 0.5) [set destination-y (0 - destination-y)]
...
to go
  ...
  ask cars [
    ...
    if isavailable [
        ...
        let accept-request where-to-go

                                 ;;negocie with other cars
                                 ;;decide the request to response
        if ((length accept-request) > 0)
           [finish-request accept-request]
    ]
...
to send-request                   ;;requests procedure
  ...
  let p []                       ;;list xcor ycor destination-x destination-y time-to-die
...
to-report where-to-go              ;;cars procedure


  let winner-id 0
  let winner-distance 100000000  ;;should be infinity
  let winner-request []
  ...
  [
     ...
     if type-msg = "request" [
          ...
          if ((timer + d1) < request-live-time)[                              ;;check car can arrive request position within request live time

              if ( d < winner-distance )[

                   set winner-id rId
                   set winner-request []                                       ;;empty winner-request
                   set winner-request lput rId winner-request                  ;;format winner-request
                   set winner-request lput dx1 winner-request                  ;;[request-id request-position-x request-position-y destination-x destination-y]
                   set winner-request lput dy1 winner-request
                   set winner-request lput dx2 winner-request
                   set winner-request lput dy2 winner-request

                   set winner-distance d
              ]
          ]
  ...
  report  winner-request                  ;;should be the request and the destination
...
to finish-request [accept-request]                                  ;;cars procedure  accepte request and do it
   ...
   let end-request (timer + d / 10)

   set isavailable false                  ;;not available
   set color red
   set time-finish-request end-request    ;;set the end of the request

   ask turtle rId [set isalive false]     ;;request has be dealed

   ;let to-request (timer + d1)            ;; wait
   ;while [timer < to-request] []

   move-to turtle rId                     ;;move to request position

   ask turtle rId [hide-turtle]
...
to creat-requests
  if (timer > time-recreate-requests)[
        ...
        set time-recreate-requests (timer + 5)    ;;5 can be replaced be a global variable
    ]
